...
export class XiaomiMiAirPurifierAccessory implements AccessoryPlugin {
  ...
  constructor(
    ...
    if (isValidConfig(config)) {
      ...
      // Air Purifier Service — required characteristics
      ...
      // Air Purifier Service — optional characteristics
      ...
      // Filter Maintenance Service — always exposed so HomeKit can track filter life
      (this as any).filterMaintenanceService = new FilterMaintenance(
        `Filter on ${this.name}`,
      );
      addFilterLifeLevel(
        this.maybeDevice,
        (this as any).filterMaintenanceService,
      ...
      if (config.enableAirQuality) {
        (this as any).airQualitySensorService = new AirQualitySensor(
          `Air Quality on ${this.name}`,
        );
        addAirQuality(
          this.maybeDevice,
          (this as any).airQualitySensorService,
      ...
      if (config.enableTemperature) {
        (this as any).temperatureSensorService = new TemperatureSensor(
          `Temperature on ${this.name}`,
        );
        addCurrentTemperature(
          this.maybeDevice,
          (this as any).temperatureSensorService,
      ...
      if (config.enableHumidity) {
        (this as any).humiditySensorService = new HumiditySensor(
          `Humidity on ${this.name}`,
        );
        addCurrentRelativeHumidity(
          this.maybeDevice,
          (this as any).humiditySensorService,
          Characteristic.CurrentRelativeHumidity,
        );
      }

      // Device Info
      (this as any).accessoryInformationService = new AccessoryInformation().setCharacteristic(
